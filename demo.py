...
voxel_num = 497
y = data[:,voxel_num+3]
x = 2*(np.pi/180)*((data[:,3]-1)*20)#spread over 2pi
...
vmf = 0.5*mcmc_tc.vm_fwhm(sampler_vm.flatchain[:,3])#0.5 bevause vm is on 2pi
vm_max, vm_min = mcmc_tc.vm_maxmin(sampler_vm.flatchain)
vmb = vm_min#'real' baseline
vma = vm_max-vm_min#'real' amplitude
